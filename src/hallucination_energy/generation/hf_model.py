"""HuggingFace causal-LM wrapper that also returns per-token log-likelihoods
and hidden-state embeddings needed by the codelength and geometry energies.

Ported from the repository root's ``huggingface_models.py``. Functionally
unchanged from the original except:

- imports ``BaseModel``/``STOP_SEQUENCES`` from the local
  ``generation.base_model`` instead of the (unvendored) upstream
  ``uncertainty.models.base_model``, removing that hidden dependency.
"""
...
import accelerate
import torch
from accelerate import Accelerator  # noqa: F401  (kept for parity with upstream import surface)
...
from hallucination_energy.generation.base_model import STOP_SEQUENCES, BaseModel
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor):
        ...
        for stop in self.stops:
            if self.match_on == "text":
                ...
                raise ValueError(f"Unknown match_on={self.match_on!r}")
            if match:
                return True
        return False
...
def remove_split_layer(device_map_in):
    ...
    for layer, count in counts.items():
        ...
        logging.info("Split layer is %s.", layer)
...
class HuggingfaceModel(BaseModel):
    """HuggingfaceModel with custom caching directory."""
    ...
    def __init__(self, model_name, stop_sequences=None, max_new_tokens=None, cache_dir=None):
        ...
        if "llama" in model_name.lower():
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                f"{base}/{model_name}", device_map="auto", token_type_ids=None, cache_dir=cache_dir
            )
            ...
            if ("7b" in model_name or "13b" in model_name) or eightbit:
                self.model = AutoModelForCausalLM.from_pretrained(
                    f"{base}/{model_name}", device_map="auto", max_memory={0: "80GIB"}, cache_dir=cache_dir, **kwargs
                )
                ...
                self.model = accelerate.load_checkpoint_and_dispatch(
                    self.model, path, device_map=full_model_device_map, dtype="float16", skip_keys="past_key_values"
                )

            else:
                raise ValueError(f"Unsupported llama variant: {model_name}")
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(
                model_id, device_map="auto", token_type_ids=None, clean_up_tokenization_spaces=False, cache_dir=cache_dir
            )

            self.model = AutoModelForCausalLM.from_pretrained(
                model_id, device_map="auto", max_memory={0: "80GIB"}, cache_dir=cache_dir, **kwargs
            )
            ...
            self.model = AutoModelForCausalLM.from_pretrained(
                model_id, trust_remote_code=True, device_map="auto", cache_dir=cache_dir, **kwargs
            )
            ...
            self.model = AutoModelForCausalLM.from_pretrained(
                model_id, trust_remote_code=True, device_map="auto", cache_dir=cache_dir
            )
            ...
            self.model = AutoModelForCausalLM.from_pretrained(
                model_id, trust_remote_code=True, device_map="auto", torch_dtype=torch.bfloat16, cache_dir=cache_dir
            )
        else:
            raise ValueError(f"Unsupported model family for model_name={model_name!r}")
    ...
    def predict(self, input_data, temperature, return_full=False, return_latent=False):
        ...
        if len(outputs.sequences[0]) > self.token_limit:
            raise ValueError("Generation exceeding token limit %d > %d" % (len(outputs.sequences[0]), self.token_limit))
